...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        public void SaveActionBinding()
        {
            ...
            var key = $"rebinds::{asset.name}::{map.name}";
        ...
        private bool IsBindingAlreadyUsed(InputAction action, int bindingIndex, string newPath)
        {
            ...
            foreach (var map in asset.actionMaps)
            {
                foreach (var act in map.actions)
                {
                    ...
                    {
                        // Skip the binding we are currently rebinding
        ...
        {
            var bindingJson = PlayerPrefs.GetString(m_Action.action.name + bindingId);
            if (!string.IsNullOrEmpty(bindingJson))
            {
                actionReference.action.actionMap.LoadBindingOverridesFromJson(bindingJson);
            }
        }
            ...
            m_RebindOperation = action.PerformInteractiveRebinding(bindingIndex)
                ...
                .OnComplete(
                    operation =>
                    {
                        ...
                        if (IsBindingAlreadyUsed(action, bindingIndex, newPath))
                        {
                            Debug.LogWarning("This key is already bound to another action!");

                            // Revert binding
                            action.RemoveBindingOverride(bindingIndex);

                            // Optional: show UI warning
                            m_RebindText.text = "Key already in use!";
                        }
